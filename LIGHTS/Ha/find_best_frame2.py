...
for ii in range(start, end+1 ,1):
    n = str(ii)
    cat = np.genfromtxt(path_images + '/data-reduction/sky-sub2/'  + obj + '_Sloan-' + filter + '_' + n + '_ccd' + h + '.txt')
    rms[ii] = cat[1]

rms_min = np.amin(rms[1:])
index = int(np.argwhere(rms == rms_min ))
filef = open(path_images + '/data-reduction/rms_best_ccd' + h + '-2.txt', "w")
print(str(index), str(rms_min), file=filef)
