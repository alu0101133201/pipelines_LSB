...
for ii in range(start, end+1 ,1):
    n = str(ii)
    cat = np.genfromtxt(path_images + '/data-reduction/noise-sky-after-photometry2/'  + obj + '_Sloan-' + filter + '_' + n + '.txt')
    rms[ii] = cat[1]

rms_min = np.amin(rms[1:])

file = open(path_images + '/data-reduction/rms_min_val-2.txt', "w")
